...
Some other notes:
- `-g` and `-t` are exclusive. Whichever was set last in the command line is the one that will be used.
- `-l` may be used alone, which displays one entry per line. It may also be used with `-g` or `-t`.
...
I'm currently using eza as my display-oriented ls alternative. It has a lot more features than just being a basic ls wrapper (e.g. tree display, gradient coloring), so I decided not to reinvent the wheel.
